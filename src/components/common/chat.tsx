...
const Chat = () => {
  useEffect(() => {
    fetch('/api/chat', {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'user', content: 'What is a 404 error?' },
          { role: 'assistant', content: 'A 404 error is a standard HTTP response code that indicates the server could not find the requested content. It is also known as "Not Found".' },
        ]
      }),
    },)
  ...
  return (
    <div style={{ boxShadow: '0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)' }}
      className="fixed bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white dark:bg-black p-6 rounded-lg border border-[#e5e7eb] dark:border-neutral-800 w-[440px] h-[600px]">

      {/* <!-- Heading --> */}
      <div className="flex flex-col space-y-1.5 pb-6">
        <h2 className="font-bold text-2xl tracking-tight">Chatty</h2>
        <p className="text-sm font-medium text-[#6b7280] dark:text-neutral-300 leading-6">
          Preguntale a Chatty cualquier duda acerca de los estados HTTP.
        </p>
      </div>

      {/* <!-- Chat Container --> */}
      <div className="pr-4 h-[70%]" style={{ minWidth: '100%', display: 'table' }}>
        {/* <!-- Chat Message AI --> */}
